...
base manifests (`deploy/kube/manifests/agent-sandbox.yaml`). Traefik is disabled at
...
3. Apply the GatewayClass manifest (the Helm chart does not create one):
   ```bash
...
| `deploy/helm/openshell/values-ingress.yaml` | Traefik Ingress overlay (Traefik disabled by default) |
| `deploy/helm/openshell/values-keycloak.yaml` | Keycloak OIDC overlay |
| `deploy/kube/manifests/envoy-gateway-openshell.yaml` | GatewayClass for Envoy Gateway (apply after Helm deploy) |
