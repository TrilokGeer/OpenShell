...
fn gateway_container_name(name: &str) -> String {
    format!("openshell-cluster-{name}")
}

/// Fetch logs from the gateway Docker container.
///
/// Connects to Docker locally or through SSH and retrieves logs from the
/// `openshell-cluster-{name}` container.
pub fn doctor_logs(
    name: &str,
    lines: Option<usize>,
    tail: bool,
    ...
    let container = gateway_container_name(name);
    ...
    let status = if let Some(ref host) = remote_host {
        validate_ssh_host(host)?;
        let mut cmd = Command::new("ssh");
        if let Some(key) = ssh_key {
            cmd.args(["-i", key]);
        }
        cmd.arg(host).arg("docker").args(&args).status()
    } else {
        Command::new("docker").args(&args).status()
    }
    .into_diagnostic()
    .wrap_err("failed to execute docker logs")?;

    if !status.success() {
        return Err(miette::miette!(
            "docker logs failed with exit code {}",
