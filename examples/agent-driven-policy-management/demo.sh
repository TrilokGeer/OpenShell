...
create_sandbox() {
    ...
    "$OPENSHELL_BIN" sandbox create \
        ...
        --policy "$POLICY_FILE" \
        --upload "${RUNNER_SOURCE}:/sandbox/policy-demo-runner.sh" \
        --no-git-ignore \
        --keep \
        --no-auto-providers \
        --no-tty \
        -- bash -lc "chmod +x /sandbox/policy-demo-runner.sh && echo sandbox ready"
}
...
run_policy_local_checks() {
    ...
    sandbox_exec /sandbox/policy-demo-runner.sh check-skill >/dev/null
    info "${GREEN}Skill installed:${RESET} /etc/openshell/skills/policy_advisor.md"

    local output
    output="$(sandbox_exec /sandbox/policy-demo-runner.sh current-policy)"
    local status
    status="$(printf '%s\n' "$output" | http_status)"
...
attempt_write() {
    sandbox_exec /sandbox/policy-demo-runner.sh put-file \
        "$DEMO_GITHUB_OWNER" \
        "$DEMO_GITHUB_REPO" \
        "$DEMO_BRANCH" \
        "$DEMO_FILE_PATH" \
...
submit_policy_proposal() {
    sandbox_exec /sandbox/policy-demo-runner.sh submit-proposal \
        "$DEMO_GITHUB_OWNER" \
        "$DEMO_GITHUB_REPO" \
        "$DEMO_FILE_PATH"
}
